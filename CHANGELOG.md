...
- **Idle unload (`MLX_IDLE_UNLOAD_S`, default 300s)**: same policy as the GGUF in-process path — weights drop after inactivity, freeing GPU/unified memory (verified live: 604MB active → 0.0MB, transparent ~1s reload, embeddings identical across the cycle). `0` disables. Mirrors the GGUF 5-minute unload so the MLX daemon is not an always-on RAM cost.
...
- **External Resource Watchdog (`scripts/qmd-mlx-watchdog.py`, `scripts/qmd_mlx/watchdog.py`)**: Standalone supervisor monitoring target MLX PID RSS, macOS system memory pressure (`memory_pressure`/`vm_stat`), swap growth (`sysctl vm.swapusage`), and HTTP `/health` probe responsiveness. Enforces strict process isolation to terminate only the verified test MLX server on breach (never unrelated system processes). Derives conservative safety ceilings dynamically from available memory headroom (not installed RAM), leaving at least 35% headroom for OS stability.
- **Dedicated Control-Path Capacity & Saturation Diagnostics**: Independent capacity limits for inference routes vs control plane in `ThreadedMLXServer` (`max_concurrent_inference`, `inference_limiter`, and `request_queue_size=128`). Saturated inference requests return immediate HTTP 429 (`OverloadedError`), while `/health` and `/ready` remain fast and responsive with HTTP 200 and live diagnostics (`queue_depth`, `in_flight_requests`, `overloaded`), preventing external restart-loop failures.
- **Stalled-Kernel Fail-Safe Deadlines & Admission Recovery**: Monotonic deadline budgets on GPU forward execution and prompt lease release upon timeouts (HTTP 504 `DeadlineExceededError`) or client cancellations (`RequestCancelledError`), enabling deterministic recovery without resource leaks.
- **Process Isolation & Start-Time Revalidation**: Watchdog revalidates target process start-time identity before EVERY signal (`SIGTERM` and `SIGKILL`), preventing signals to recycled PIDs. Subprocess launch mode allows watchdog to own and monitor test fixture children without touching external system processes.
- **Fail-Closed System Telemetry & Headroom Safety**: Memory sampler fails closed upon telemetry errors (`SystemMetricsError`), eliminating optimistic fallback hallucinations. Headroom excludes purgeable double-counting, and tight headroom dynamic clamping prevents exceeding actual available RAM.
- **Worker Progress Telemetry & Stuck Inference Detection**: Added live execution tracking (`active_job_age_s`, `completed_sequence`, `worker_alive`, `worker_idle`, `pid`, `instance_token`) to `GPUExecutor` and HTTP `/health`. Watchdog detects wedged GPU worker execution even when HTTP `/health` returns 200 OK.
- **HTTP Protocol Framing Protection on Saturated 429**: Saturated POST rejections send `Connection: close` and terminate socket streams, eliminating protocol framing desync on keepalive connections.
- **Cold Model Init Health Probing**: Server serves HTTP `/health` immediately during model loading in `ServerState.STARTING` / `LOADING` with startup grace period policy.
- **Phase 3 Single-Model Smoke Qualification Harness (`scripts/qmd-mlx-smoke.py`, `scripts/qmd_mlx/smoke.py`)**: Standalone qualification harness with model-aware headroom preflight (deriving memory minimums from model parameters, weights, and activation margins >= 3500MB for 4B models), active `MLXWatchdog` process ownership and background supervision, absolute wall-clock timeout ceilings, proxy-isolated bounded HTTP probe client (`trust_env=False`, 10MB chunked body cap), and full test suite verification.
- **Synthetic Embedding Adapter (`SyntheticEmbeddingAdapter`)**: Offline embedding adapter generating deterministic hash-seeded unit vectors within the real server stack (`scripts/mlx_embed_server.py`) for complete plumbing, concurrency, and watchdog rehearsals without loading real weights.

### Fixed
- Fixed Phase 3 smoke harness composition: replaced fake server imitation with real server stack rehearsal via `SyntheticEmbeddingAdapter`, added active `MLXWatchdog` background supervision, eliminated `subprocess.PIPE` deadlock risks via managed temporary logs, and enforced outer monotonic wall-clock timeout ceilings on HTTP probes.
...
- Whole-architecture reliability pass (Sep 2026, experimental MLX path only — GGUF production untouched): single GPU execution owner with fair interactive/bulk interleaving and monotonic end-to-end deadlines; shared residency budget with evict-before-load, single-flight loads, and idle unload on all three stages; fixed 3-model preload ordering (register-before-load); bounded generation decode with cancel/deadline checks; template-pinned rerank yes/no IDs; measured (not labeled) quantization/revision in descriptors; explicit oversize/empty-input errors instead of silent truncation; partial-tail resume via chunk expectations; fingerprint-mismatch errors instead of auto-wipe; strict all-MLX mode; atomic clears; `busy_timeout`; SIGINT-safe durable jobs.
- Fixed MLX rerank logits calculation on `QuantizedLinear` heads (avoiding slice indexing on packed `uint32` weights and evaluating head over last-token hidden state).
- Fixed fail-closed fallback semantics in `LlamaCpp` when `embedBackend === 'mlx'`, preventing unintended fallback to GGUF and vector space cross-contamination.
- Fixed GPUExecutor cancellation propagation across all exception pathways when awaiting submitted futures.

...
def main() -> int:
    ...
    t0 = time.time()
    adapter = MLXRerankAdapter(model_name="/Users/shersingh/.cache/qmd/models/qwen3-reranker-4b-mlx-8bit")
    load_s = time.time() - t0
    adapter.warmup()
    ...
    verdict = "PASS" if (top1_agreement >= 0.90 and mean_rho >= 0.80) else "FAIL"
    result = {
    ...
    out = REPO / "docs/benchmarks/mlx_rerank_parity_8bit.json"

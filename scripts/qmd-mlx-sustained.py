#!/usr/bin/env python3
"""
qmd-mlx-sustained.py — Bounded Sustained MLX Embedding Qualification Runner

Executes bounded pilot (<=30 requests, <=120s) and bounded soak (<=100 requests, <=180s)
benchmarks across four verified token strata (short, medium, long, code) under active
MLXWatchdog supervision with strict process isolation and detailed JSON measurement reporting.

Usage:
    # 1. Offline Rehearsal (Pilot Mode, Fake Adapter, <=30 requests):
    python3 scripts/qmd-mlx-sustained.py --rehearsal --mode pilot --json

    # 2. Offline Rehearsal (Soak Mode, Fake Adapter, <=100 requests):
    python3 scripts/qmd-mlx-sustained.py --rehearsal --mode soak --json

    # 3. Real-Model Bounded Pilot (Explicit opt-in, <=30 requests, <=120s):
    python3 scripts/qmd-mlx-sustained.py --real-model \
        --model-path ~/.cache/qmd/models/qwen3-embedding-4b-mlx-4bit-affine \
        --mode pilot \
        --output-file docs/reviews/artifacts/phase3-pilot-report.json

    # 4. Real-Model Bounded Soak (Explicit opt-in, <=100 requests, <=180s):
    python3 scripts/qmd-mlx-sustained.py --real-model \
        --model-path ~/.cache/qmd/models/qwen3-embedding-4b-mlx-4bit-affine \
        --mode soak \
        --output-file docs/reviews/artifacts/phase3-soak-report.json
"""
...
def main(argv: Optional[list[str]] = None) -> int:
    parser = argparse.ArgumentParser(
        description="Bounded Sustained MLX Embedding Qualification Runner",
    ...
    parser.add_argument(
        "--rehearsal",
        action="store_true",
        help="Run offline rehearsal with disposable synthetic adapter (NO real weights loaded)",
    )
    parser.add_argument(
        "--real-model",
        action="store_true",
    ...
    parser.add_argument(
        "--skip-pilot-gate",
        action="store_true",
        help="Bypass pilot qualification gate check before soak mode (not recommended for unattended runs)",
    )
    parser.add_argument(
        "--pilot-report",
        default="docs/reviews/artifacts/phase3-pilot-report.json",
        help="Path to completed pilot report to verify before running soak mode",
    )
    parser.add_argument(
        "--output-file",
        default=None,
    ...
    # Gate check: prevent premature soak escalation if pilot failed
    if mode == "soak" and args.real_model and not args.skip_pilot_gate and not args.dry_run:
        pilot_path = os.path.expanduser(args.pilot_report)
        if not os.path.exists(pilot_path):
            print(
                f"Error: Pilot qualification report '{pilot_path}' not found. "
                f"Run and verify pilot qualification before escalating to soak mode, "
                f"or pass --skip-pilot-gate to override.",
                file=sys.stderr,
            )
            return 2
        try:
            with open(pilot_path, "r", encoding="utf-8") as pf:
                pilot_data = json.load(pf)
            qual = pilot_data.get("qualification_evaluation", {})
            if qual.get("overall_status") != "passed":
                print(
                    f"Error: Pilot qualification '{pilot_path}' recorded status '{qual.get('overall_status', 'failed')}'. "
                    f"Failure reasons: {qual.get('failure_reasons')}. "
                    f"Refusing to escalate to soak mode without a passing pilot. Pass --skip-pilot-gate to override.",
                    file=sys.stderr,
                )
                return 2
        except Exception as p_err:
            print(f"Error reading pilot report '{pilot_path}': {p_err}", file=sys.stderr)
            return 2
    ...
    if args.output_file:
        try:
            ...
            print(f"[qmd-mlx-sustained] JSON report written to {args.output_file}")
        except Exception as e:
    ...
    if args.json:
        ...
        print("\n=== MLX Sustained Embedding Qualification Report ===")
        ...
        if mem:
            print(f"\nMetal Unified Memory Telemetry:")
            print(f"  Peak Active Metal:           {mem.get('peak_active_mb', 0):.1f} MB")
